...
//         // 1. Fetch user's complete order history
...
//         // 3. Build purchase profile from order history
//         let purchaseProfile = {
...
//                 if (item._id) purchaseProfile.purchasedProductIds.push(item._id.toString());

//                 if (item.category) {
//                     purchaseProfile.categories[item.category] =
//                         (purchaseProfile.categories[item.category] || 0) + (item.quantity || 1);
//                 }

//                 if (item.subCategory) {
//                     purchaseProfile.subCategories[item.subCategory] =
//                         (purchaseProfile.subCategories[item.subCategory] || 0) + (item.quantity || 1);
//                 }

//                 if (item.price) {
//                     purchaseProfile.priceRange.min = Math.min(purchaseProfile.priceRange.min, item.price);
//                     purchaseProfile.priceRange.max = Math.max(purchaseProfile.priceRange.max, item.price);
//                 }
...
//         // Fix infinite min if no orders
//         if (purchaseProfile.priceRange.min === Infinity) {
//             purchaseProfile.priceRange = { min: 0, max: 0, avg: 0 };
//         }

//         // Calculate average price
//         const allPrices = orders.flatMap(o => o.items.map(i => i.price || 0)).filter(p => p > 0);
//         purchaseProfile.priceRange.avg = allPrices.length > 0
//             ? Math.round(allPrices.reduce((a, b) => a + b, 0) / allPrices.length)
//             : 0;

//         // 4. Simplified product catalog for Gemini (avoid token overflow)
...
//         if (isNewUser) {
//             prompt = `You are a smart fashion recommendation AI for an e-commerce clothing store called "Forever".
...
// Your task: Recommend exactly 8 products that would be great starting picks for a new customer — a mix of bestsellers, popular categories, and good value items.

// IMPORTANT: Return ONLY a valid JSON object. No explanation. No markdown. No code blocks. Just raw JSON:
// {"recommendedIds":["id1","id2","id3","id4","id5","id6","id7","id8"],"reason":"One sentence about why these are good starter picks"}`;
//         } else {
//             prompt = `You are a smart fashion recommendation AI for an e-commerce clothing store called "Forever".

// USER PURCHASE PROFILE:
// - Total orders: ${purchaseProfile.totalOrders}
// - Total spent: ₹${purchaseProfile.totalSpent}
...
// - Price range: ₹${purchaseProfile.priceRange.min} - ₹${purchaseProfile.priceRange.max} (avg ₹${purchaseProfile.priceRange.avg})
// - Recent purchases: ${JSON.stringify(purchaseProfile.recentItems)}
// - Already purchased IDs (DO NOT recommend these): ${JSON.stringify(purchaseProfile.purchasedProductIds)}

// AVAILABLE PRODUCTS:
// ${JSON.stringify(productCatalog, null, 2)}

// YOUR TASK:
// 1. Analyze the user's shopping patterns (categories, price sensitivity, style preferences)
// 2. Recommend exactly 8 products they are most likely to love
// 3. Prioritize same/related categories, similar price range, complementary items
// 4. NEVER recommend already-purchased products
// 5. Include 1-2 bestsellers if relevant

// IMPORTANT: Return ONLY a valid JSON object. No explanation. No markdown. No code blocks. Just raw JSON:
// {"recommendedIds":["id1","id2","id3","id4","id5","id6","id7","id8"],"reason":"One sentence explaining the recommendation logic"}`;
//         }

//         // 6. Call Gemini API — using gemini-1.5-flash (fast + free tier friendly)
...
//                 topCategory: Object.keys(purchaseProfile.categories).sort(
//                     (a, b) => purchaseProfile.categories[b] - purchaseProfile.categories[a]
//                 )[0] || null,
...
//     } catch (error) {
//         console.log("Gemini Recommendation Error:", error);

//         // Specific Gemini API error messages
//         if (error.message?.includes('API_KEY')) {
//             return res.json({ success: false, message: "Gemini API key is invalid or missing." });
//         }
//         if (error.message?.includes('quota')) {
//             return res.json({ success: false, message: "Gemini API quota exceeded. Please try again later." });
//         }
...
{"recommendedIds":["id1","id2","id3","id4","id5","id6","id7","id8"],"reason":"brief reason here"}`;
        ...
        if (error.message?.includes('API_KEY') || error.message?.includes('api key')) {
            return res.json({ success: false, message: "Gemini API key is invalid or missing. Check GEMINI_API_KEY in .env" });
        }
        if (error.message?.includes('quota') || error.message?.includes('429')) {
            return res.json({ success: false, message: "Gemini API quota exceeded. Please try again in a minute." });
        }
        if (error.message?.includes('404') || error.message?.includes('not found')) {
            return res.json({ success: false, message: "Gemini model not found. Make sure your API key has access to gemini-1.5-flash." });
        }
