...
//     return jwt.sign({id},process.env.JWT_SECRET_KEY,{ expiresIn: "7d" } // new
// );
// }
...
const changePassword = async (req, res) => {
    try {
        ...
        if (isSame) {
            return res.json({ success: false, message: "New password cannot be the same as current password." });
        }
